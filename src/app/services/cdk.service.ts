...
export class CdkService {
  ...
  private viewContainerRef!: ViewContainerRef; // armazenamos aqui
  ...
  serializeComponent(element: HTMLElement): CdkComponent {
    ...
    return {
      id: element.id,
      type: element.classList[1] || "component",
      styles,
  ...
  serializeProject(rootElements: HTMLElement[]): ProjectInterface {
    return {
      urlLogo: "url_aqui",
      title: "Titulo",
      subTitle: "Subtitulo",
      description: "Descrição",
      component: rootElements.map(el => this.serializeComponent(el))
    };
  }
  ...
  saveProject(): void {
    ...
    this.dropZonesIds.forEach(id => {
      ...
      if (el) {
        Array.from(el.children).forEach(child => rootElements.push(child as HTMLElement));
      }
    });

    const projeto = this.serializeProject(rootElements);
